...
\begin{abstract}
Gardner et al.\ (2022) demonstrated that the joint population activity of grid cells in the medial entorhinal cortex resides on a toroidal manifold $T^2 = S^1 \times S^1$, providing the first direct empirical evidence that neural computation evolves on non-trivial topological attractor geometries.
...
This result was significant for several reasons. It provided the first direct visualization of continuous attractor network dynamics in the brain. It confirmed that neural population activity can be organized on non-trivial topological manifolds. And it demonstrated that the tools of algebraic topology can be applied to experimental neural data to recover geometric structure that is not visible in the activity of individual neurons.
...
Neural systems contain precisely the oscillatory dynamics needed to produce toroidal attractors. Electroencephalographic recordings reveal rhythmic activity spanning multiple frequency bands, from slow delta oscillations ($<4$~Hz) through theta (4--8~Hz), alpha (8--13~Hz), beta (13--30~Hz), and gamma ($>30$~Hz) rhythms \cite{Buzsaki2006, Buzsaki2004}. These oscillations interact through cross-frequency coupling mechanisms, notably theta-gamma phase-amplitude coupling observed in the hippocampus and neocortex \cite{Canolty2006, Tort2009, Lisman2005}. Such coupled oscillatory dynamics provide the conditions under which toroidal attractor manifolds are expected to emerge.

\subsection{Goal of This Paper}

This paper develops a physical dynamical framework explaining why toroidal manifolds arise in recursive neural systems. The argument proceeds through several layers: the electromagnetic spectrum as the base physical layer, the phase space geometry of coupled oscillatory systems, the grid cell evidence as empirical anchor, quaternion rotations as mathematical representation of multi-band phase dynamics, entropy gradients as organizing principles, evolutionary pressure from the gut microbiome as a driver of recursive internal organization, and the transition beyond the primitive consciousness state described by Jaynes \cite{Jaynes1976}. The conclusion connects these threads to recursive self-reference consistent with G\"{o}del-type logical structure \cite{Godel1931, Hofstadter1979}.
...
The key observation is that a large measurement gap exists between the frequencies accessible through EEG and the much higher frequencies associated with molecular and lattice dynamics in neural tissue. Current neuroscience instrumentation was not designed to detect signals in this intermediate range. The terahertz band is of particular interest because it corresponds to collective vibrational modes of biological structures, and THz spectroscopy may reveal previously unobserved electromagnetic dynamics in neural tissue that are relevant to the synchronization landscape of macroscopic oscillatory activity.
...
\end{equation}
where $\omega_i$ is the angular frequency and $\phi_i$ is the initial phase.
...
\begin{equation}\label{eq:torus}
  T^2 = S^1 \times S^1,
\end{equation}
which is the two-dimensional torus.
...
\end{align}
where $R$ is the major radius (distance from the center of the torus to the center of the tube) and $r$ is the minor radius (radius of the tube), with $\theta, \phi \in [0, 2\pi)$ corresponding to the two independent phase variables.
...
The KAM theorem states that for a Hamiltonian system with $n$ degrees of freedom, if the unperturbed system is integrable (meaning it possesses $n$ independent constants of motion in involution), then most invariant tori carrying quasi-periodic motion of the form~\eqref{eq:quasiperiodic} survive under sufficiently small perturbations, provided the frequency vector satisfies a Diophantine condition:
...
Having introduced the grid cell system and the Gardner et al.\ result in Section~\ref{sec:intro}, this section provides a more detailed treatment of the empirical evidence and its implications for the toroidal attractor framework.

Grid cells, first described by Hafting et al.\ \cite{Hafting2005} in the dorsocaudal medial entorhinal cortex (dMEC), fire whenever the animal's position coincides with a vertex of a regular triangular (hexagonal) lattice that tiles the environment.
The spatial firing pattern of a single grid cell is characterized by three parameters: the spacing between grid vertices, the orientation of the grid axes, and the spatial phase (the offset of the grid pattern).
...
Stensola et al.\ \cite{Stensola2012} demonstrated that the ratio of grid spacings between adjacent modules is approximately constant, following a geometric progression with a scale factor of roughly 1.4.
...
As described in Section~\ref{sec:intro}, Gardner et al.\ \cite{Gardner2022} used persistent cohomology to reconstruct the topology of the manifold on which grid cell population activity resides, revealing a two-dimensional torus $T^2 = S^1 \times S^1$ (see Figure~\ref{fig:toroidal_manifold}).
The methodological details of this result merit further discussion.

Gardner et al.\ recorded from large populations of neurons using Neuropixels silicon probes implanted in the medial entorhinal cortex of freely moving rats.
The high-density recordings captured the simultaneous activity of hundreds of grid cells within individual modules.
To reconstruct the topology of the population activity manifold, they applied persistent cohomology, a technique from topological data analysis that identifies topological features (connected components, loops, voids) that persist across multiple spatial scales in a point cloud.
The key insight is that persistent cohomology can detect the presence of non-trivial topology without requiring an explicit embedding or dimensionality reduction step.
Applied to the grid cell population vectors, the analysis revealed two independent circular cohomology classes, which is the algebraic signature of a torus $T^2 = S^1 \times S^1$.

The robustness of this result was confirmed by several controls.
The toroidal topology persisted across different recording sessions, different animals, and different behavioral conditions.
Shuffled data, in which the temporal relationships between neurons were destroyed, did not produce toroidal topology.
The result was also consistent across different parameter choices in the persistent cohomology pipeline, indicating that the toroidal structure is a genuine feature of the data rather than an artifact of the analysis method.
...
The significance extends beyond spatial navigation: it establishes that neural population activity can be organized on non-trivial topological manifolds recoverable through algebraic topology.
...
The specific historical claims of Jaynes's theory remain debated, and the present paper does not depend on their accuracy.
...
Whether or not this transition occurred on the historical timeline Jaynes proposed, the distinction between a neural architecture that cannot distinguish internally generated signals from external ones and an architecture that can is a genuine computational distinction with physical consequences.
...
However, several authors, most notably Hofstadter \cite{Hofstadter1979} and Penrose \cite{Penrose1989}, have observed that the structural features underlying G\"odel's result, specifically the capacity for self-reference in sufficiently complex recursive systems, have parallels in neural computation.
...
To be precise: we do not claim that neural systems are formal systems, or that G\"odel's theorems apply directly to neural computation.
...
As Hofstadter \cite{Hofstadter1979} argued, the recursive self-referential structure is not incidental to complex systems but a characteristic feature of systems capable of introspective representation.
...
And the recursive self-reference enabled by modular oscillatory organization, shaped by evolutionary pressure for internal modeling, provides the computational architecture for introspective consciousness.
