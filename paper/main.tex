...
\caption{Grid cell hexagonal firing pattern. (A) Spatial firing fields of a single grid cell, showing elevated firing rates at regularly spaced locations forming a hexagonal lattice within the arena. (B) The hexagonal lattice structure defined by two basis vectors $\mathbf{e}_1$ and $\mathbf{e}_2$ separated by $60°$, with characteristic grid spacing $\lambda$. (C) The periodic spatial code wraps each spatial direction modulo $\lambda$, producing two cyclic phase variables $\phi_1, \phi_2 \in S^1$. Their joint state space is the two-torus $T^2 = S^1 \times S^1$.}
...
The KAM theorem addresses Hamiltonian systems with $n$ degrees of freedom. If the unperturbed system is integrable, then most invariant tori carrying quasi-periodic motion of the form~\eqref{eq:quasiperiodic} survive under sufficiently small perturbations. The key requirement is that the frequency vector satisfies a Diophantine condition:
...
As Hofstadter \cite{Hofstadter1979} argued, the recursive self-referential structure is a characteristic feature of systems capable of introspective representation.
